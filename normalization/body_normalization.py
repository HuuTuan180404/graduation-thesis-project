...
def normalize_body_full(df: pd.DataFrame) -> (pd.DataFrame, list):
    ...
    for index, row in df.iterrows():
        ...
        valid_sequence = True
        original_row = row
        logging.info(f'ZZZZZ {sequence_size}')
        ...
        for sequence_index in range(sequence_size):
            ...
            if (row["leftShoulder_X"][sequence_index] == 0 or row["rightShoulder_X"][sequence_index] == 0) and (row["neck_X"][sequence_index] == 0 or row["nose_X"][sequence_index] == 0):
                ...
                if row["leftShoulder_X"][sequence_index] != 0 and row["rightShoulder_X"][sequence_index] != 0:
                    ...
                    neck = (row["neck_X"][sequence_index], row["neck_Y"][sequence_index]) # TỌA ĐỘ CỦA CỔ C(X,Y)
                    nose = (row["nose_X"][sequence_index], row["nose_Y"][sequence_index]) # TỌA ĐỘ CỦA MŨI M(X,Y)
                    neck_nose_distance = ((((neck[0] - nose[0]) ** 2) + ((neck[1] - nose[1]) ** 2)) ** 0.5) # TÍNH KHOẢNG CỦA CM ECLID
...
def normalize_single_dict(row: dict):
    ...
    for sequence_index in range(sequence_size):
        ...
        # Prevent from even starting the analysis if some necessary elements are not present
        if (row["leftShoulder"][sequence_index][0] == 0 or row["rightShoulder"][sequence_index][0] == 0) and (row["neck"][sequence_index][0] == 0 or row["nose"][sequence_index][0] == 0):
            if not last_starting_point:
                valid_sequence = False
                continue
            ...
            # CRộng = 6 * head_metric (từ cổ - 3*đầu đến cổ + 3*đầu), được căn giữa theo chiều ngang quanh cổ.
            # CDài = 6 * head_metric, bắt đầu từ vị trí bên dưới mắt trái.
            starting_point = [row["neck"][sequence_index][0] - 3 * head_metric,
