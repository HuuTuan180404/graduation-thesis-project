    ...
    if "nose_X" in sign_eval:
        body_landmarks = {
            identifier: [
                (x, y)
                for x, y in zip(
                    sign_eval[identifier + "_X"], sign_eval[identifier + "_Y"]
                )
            ]
        ...
        body_landmarks = {
            identifier: sign_eval[identifier] for identifier in BODY_IDENTIFIERS
        }
        hand_landmarks = {
            identifier: sign_eval[identifier] for identifier in HAND_IDENTIFIERS
        }
...
def augment_rotate(sign: dict, angle_range: tuple) -> dict:
    ...
    body_landmarks = {
        key: [__rotate((0.5, 0.5), frame, angle) for frame in value]
...
def augment_shear(sign: dict, type: str, squeeze_ratio: tuple) -> dict:
    ...
    augmented_landmarks = cv2.perspectiveTransform(
        np.array(landmarks_array, dtype=np.float32), mtx
    )
...
def augment_arm_joint_rotate(
    sign: dict, probability: float, angle_range: tuple
) -> dict:
    """
