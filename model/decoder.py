    ...
    def forward(
        ...
        if training or self.patience == 0:
            ...
            for i, mod in enumerate(self.layers):
                ...
                label = int(
                    torch.argmax(torch.nn.functional.softmax(classifier_out, dim=2))
                )
                ...
                if patient_result is not None:
                    ...
                    patient_label = int(
                        torch.argmax(torch.nn.functional.softmax(patient_result, dim=2))
                    )

                if (patient_result is not None) and (
                    patient_label == label
                ):  # torch.all(label.eq(patient_label)):
                    patient_counter += 1
                else:
                    patient_counter = 0
